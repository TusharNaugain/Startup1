...
async function fetchNews() {
    ...
    try {
        const response = await fetch('/api/fetch_news', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ topic, country, start_date: startDate, end_date: endDate })
        });
        const data = await response.json();
...
function downloadNewsCSV() {
    ...
    let csv = 'Published Date,Source,Title,URL\n';
    globalNewsResults.forEach(row => {
        const title = row.Title.replace(/"/g, '""');
        csv += `"${row.Published}","${row.Source}","${title}","${row.Link}"\n`;
    });
    const blob = new Blob([csv], { type: 'text/csv' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
